...
const AdminNavbar = () => {
  ...
  return (
    <aside className="fixed top-0 left-0 h-screen w-52 bg-primary p-5 text-white">
      ...
      <nav>
        <ul>
          {links.map((link) => (
            <li
              key={link.id}
              className="mb-7 flex items-center space-x-3 hover:underline hover:underline-offset-8"
            >
              {link.icon}
      ...
      <section>
        <button
          className="mt-28 w-full rounded-md bg-primaryHover py-2 text-sm transition hover:bg-purple-900"
